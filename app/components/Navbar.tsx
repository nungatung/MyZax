  ...
  return (
    <>
      <div className="fixed top-0 left-0 right-0 z-[100] flex justify-center pt-4 md:pt-6 pointer-events-none">
        ...
        {/* THE PILL: Kept wide on iPads (lg instead of md) to prevent crowding */}
        <nav className={`
          ...
          ${isScrolled ? 'w-[92%] lg:w-[65%] py-1.5' : 'w-[95%] lg:w-[85%] py-2 lg:py-3'}
        `}>

          {/* LOGO: Kept at mobile scale until 'lg' screens to fit iPad width */}
          <Link href="/" className="transition-transform hover:scale-105 active:scale-95 shrink-0">
            <img 
              ...
              className={`
                ...
                ${isScrolled ? 'h-10 lg:h-14' : 'h-12 lg:h-24'} 
              `} 
            />
          </Link>

          {/* NAV LINKS: Switched from md:flex to lg:flex to hide on iPads */}
          ...
          {/* ACTIONS: Scaled gap dynamically for tablets */}
          <div className="flex items-center gap-1.5 lg:gap-6">
            ...
            {user ? (
              ...
              <button 
                ...
                className="hidden lg:block text-[13px] font-black uppercase tracking-widest text-white bg-zax-green px-7 py-2.5 rounded-full hover:bg-zax-dark transition-all cursor-pointer shadow-md"
              >
                Login
              </button>
            )}

            {/* MOBILE MENU TOGGLE: Now visible on tablets (up to 1024px) */}
            <button 
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="lg:hidden flex flex-col gap-1 p-2.5 rounded-xl bg-white/10 border border-white/20"
      ...
      {/* MOBILE MENU: Changed md:hidden to lg:hidden so it works on iPads */}
      <div className={`fixed inset-0 bg-white z-[105] flex flex-col items-center justify-center transition-all duration-700 ease-in-out lg:hidden ${
        isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none translate-y-4'
      }`}>
        {/* ... (rest of the mobile menu remains same) ... */}
